...
def getSeriesInfo(ids, data):
    ...
    seriesname = (dict.getElementsByTagName('name')[0].toxml()
        .replace("<name>", "").replace("</name>", ""))
    network = (dict.getElementsByTagName('network')[0].toxml()
        .replace("<network country=US>", "").replace("</network>", ""))
    airtime = (dict.getElementsByTagName('airtime')[0].toxml()
...
def fetchdata():
    ...
    for ids in series_ids:
        ...
        temp_thread = (threading.Thread(target=getSeriesInfo,
            args=(ids, data)))
